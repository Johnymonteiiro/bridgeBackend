...
var CalculateUseCase = class {
  constructor(primeRepository2) {
    this.primeRepository = primeRepository2;
  }
  async execute({
    number,
    execution_time,
...
// src/teste/unity/unit-result.spec.ts
var primeRepository;
var sut;
describe("Result Route", () => {
  ...
  it("Should be able to show the result", async () => {
    ...
    globalExpect(lists.length).toEqual(1);
  });
  it("Should be able to list all prime numbers result", async () => {
    ...
    const lists = primeRepository.lists;
    globalExpect(lists.length).toBeGreaterThan(0);
  });
});
